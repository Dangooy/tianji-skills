...
✓ DIN 标准覆盖：[标准列表]
✓ 尺寸补全率：X/N（已补全/总行数）
✓ 尺寸留空待人工填写：[行号+规格列表，参考表查不到的规格一律留空，不得编数]
✓ 报价列（黄色）：已标记
──────────────────────────────────
```
...
**数据门禁（全局硬规则，任何行业沿用本 skill 均适用）：尺寸数据只允许两个来源——询盘原文、`references/din_dimensions.md` 查表。表中查不到的规格，尺寸单元格一律留空，由用户自行填写，并在输出验证 Checklist 中列出留空的行。严禁凭模型知识/记忆填数，严禁按相邻规格插值估算。**

- 询盘原文已有尺寸（如 `8,4 x 16,0 x 1,6`）→ 直接提取
- 只有规格号（如 `M8` / `D8`）→ 查阅 `references/din_dimensions.md` 补全；查不到 → 留空待用户填写
