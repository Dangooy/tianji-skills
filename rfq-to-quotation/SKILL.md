...
# .xls 用 xlrd；.xlsx 用 pandas
wb = xlrd.open_workbook('询盘文件.xls')
...
def identify_columns(sheet):
    ...
    for col_idx, header in enumerate(headers):
        col_data = [sheet.cell_value(r, col_idx) for r in range(1, sheet.nrows)]
        non_empty = [v for v in col_data if str(v).strip()]
...
| SPRING WASHER | 波形弹簧垫圈 |
| Tightening washer | 碟形弹簧垫圈 |
| ZP / zp | 镀锌 |
| HEAVY | 重型 |
...
| DIN 471 | 轴用弹性挡圈 | 轴径/外径/厚度 |
| DIN 472 | 孔用弹性挡圈 | 孔径/内径/厚度 |
...
- DIN 471/472 挡圈需写出：配合直径 + 外径（或内径）+ 厚度
- **尺寸为新增列**，插入产品描述列之后
...
```
1. python -c "import xlrd, openpyxl" 2>/dev/null || pip install xlrd openpyxl -q
